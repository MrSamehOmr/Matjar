...
namespace UserControls
{
    public partial class UC_UpdateUnitsContentsInfo : UserControl
    {
        ...
        {
            string sql = string.Format(@"SELECT product_name_id, product_name FROM products_names WHERE category_id = {0} ORDER BY product_name", combo_categories.SelectedValue.ToString());
            DataTable products = db.ExecuteSQL(sql);

            combo_products_names.DataSource = products;
        ...
        {
            string units_info_query = string.Format(@"
